...
class SelectorDialog(var onSubmit: ((List<SelectionModel?>) -> Unit)? = null) {
    lateinit var selectionAdapter: SelectionAdapter

    fun showSelectorDialog(context: Context, RealLists: ArrayList<SelectionModel>) {
        val list = RealLists
        ...
        if (context is Fragment) {
            if ((context as Fragment).isAdded)
                dialog.show()
        } else
            dialog.show()
    }
